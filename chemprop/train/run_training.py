...
def run_training(args: TrainArgs,
    ...
    if args.calc_train_auc:
        train_score_data_loader = MoleculeDataLoader(
            dataset=train_data,
            batch_size=args.batch_size,
            num_workers=num_workers,
        )
    ...
    for model_idx in range(args.ensemble_size):
        ...
        for epoch in trange(args.epochs):
            ...
            if args.calc_train_auc:   # CHANGED
                train_scores = evaluate(
                    model=model, 
                    data_loader=train_score_data_loader,
                    num_tasks=args.num_tasks,
                    metrics=args.metrics,
                    dataset_type=args.dataset_type,
                    scaler=scaler,
                ...
                for vals, trains in zip(val_scores.items(), train_scores.items()):
                    metric, val_score = vals
                    _, train_score = trains
                    # Average validation score
                    avg_train_score = np.nanmean(train_score)
                    avg_val_score = np.nanmean(val_score)
                    debug(f'Train {metric} = {avg_train_score:.6f}, Validation {metric} = {avg_val_score:.6f}, N_iter = {n_iter}')  # CHANGED
                    writer.add_scalar(f'validation_{metric}', avg_val_score, n_iter)
            else:  
                for metric, scores in val_scores.items():
                    # Average validation score
                    avg_val_score = np.nanmean(scores)
                    debug(f'Validation {metric} = {avg_val_score:.6f}, N_iter = {n_iter}') 
                    writer.add_scalar(f'validation_{metric}', avg_val_score, n_iter)

                    #if args.show_individual_scores: (CHANGED)
        ...
        for metric, scores in test_scores.items():
            ...
            info(f'Model {model_idx} test {metric} = {avg_test_score:.6f}, N_iter = {n_iter}')
