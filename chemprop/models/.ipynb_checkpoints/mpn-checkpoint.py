...
# +
# Add a additional feature dimension reduction layer
...
#         self.activation = get_activation_function(args.activation)
        self.ft_layer = nn.Linear(args.features_size, 256)  # No activation?
#         dropout = nn.Dropout(args.dropout)
    def forward(self,
                feature_batch: List[np.ndarray]) -> torch.FloatTensor:
        feature_batch = torch.from_numpy(np.stack(feature_batch)).float().to(self.device)
        out = self.ft_layer(feature_batch)
        return out
...
class MPN(nn.Module):
    ...
    def __init__(self,
        ...
        self.feature_dimension_reduction = args.feature_dimension_reduction
    ...
    def forward(self,
                ...
                features_batch: List[List[np.ndarray]] = None,  # CHANGED
        ...
        :param features_batch: A list of lists of numpy arrays containing additional features.
