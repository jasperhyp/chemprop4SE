...
from chemprop.args import PredictArgs, TrainArgs, CommonArgs
...
def get_smiles(path: str,
    ...
    return smiles  # in our case output: [n_pairs, 2]
...
def get_data(path: str,
             ...
             mono_path: str = None) -> MoleculeDataset:  # Changed
    """
    ...
    with open(path) as f:
        ...
        if feature_dimension_reduction=='svd':
            mono_path = "data/mono_svd256.csv"
        
        monoSE_raw = pd.read_csv(mono_path)
        
        monoSE_dict = dict([(monoSE_raw['smiles'][i], monoSE_raw.iloc[i].values[1:].astype(np.float32)) \
                            for i in range(len(monoSE_raw))])  # Changed
...
def get_data_from_smiles(smiles: List[List[str]],
    ...
    mono_path = mono_path if mono_path is not None else os.path.join(os.curdir,"data/mono.csv")
    
    if feature_dimension_reduction=='svd':
        mono_path = "data/mono_svd256.csv"
    
    monoSE_raw = pd.read_csv(mono_path)
    monoSE_dict = dict([(monoSE_raw['smiles'][i], monoSE_raw.iloc[i].values[1:].astype(np.float32)) for i in range(len(monoSE_raw))])
...
def validate_data(data_path: str) -> Set[str]:
    ...
    with open(data_path) as f:
        ...
        smiles, targets = [], []
        for line in reader:
            smiles.append(line[:2])  # changed
            targets.append(line[2:])

    # Validate header
    if len(header) == 0:
        errors.add('Empty header')
    elif len(header) < 3:  # changed
