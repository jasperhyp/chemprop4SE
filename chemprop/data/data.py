...
class MoleculeDatapoint:
    ...
    def __init__(self,
                 ...
                 data_weight: float = 1,
                 features: List[np.ndarray] = None,  # CHANGED
        ...
        if self.features_generator is not None:
            ...
            for fg in self.features_generator:
                ...
                for s, m  in zip(self.smiles, self.mol):  # CHANGED
            ...
            # self.features = np.array(self.features)  # CHANGED

        # Fix nans in features  # CHANGED!
        # replace_token = 0
    ...
    def set_features(self, features: np.ndarray) -> None:
        ...
        :param features: A list of 1D numpy arrays of features for the molecule. # CHANGED
        """
        self.features = features
    ...
    def extend_features(self, features: np.ndarray) -> None:
        ...
        """
        self.features = self.features.append(features) if self.features is not None else [features]  # CHANGED
...
class MoleculeDataset(Dataset):
    ...
    def __init__(self, data: List[MoleculeDatapoint]):
        ...
        """
        self._data = data  # A list of MolecularDatapoints.
    ...
    def batch_graph(self) -> List[BatchMolGraph]:
        ...
        if self._batch_graph is None:
            ...
            for d in self._data:
                ...
                mol_graphs.append(mol_graphs_list)  # mol_graphs: [[mol1, mol2], [mol1, mol2], [mol1, mol2], ...]

            self._batch_graph = [BatchMolGraph([g[i] for g in mol_graphs]) for i in range(len(mol_graphs[0]))]   # [[BatchMolGraph1], [BatchMolGraph2]]

        return self._batch_graph

    # def features(self) -> List[np.ndarray]:
    def features(self) -> List[List[np.ndarray]]:  # CHANGED!
        """
        ...
        return [d.features for d in self._data]  # [[f1, f2], [f1, f2], [f1, f2], ...]
    ...
    def features_size(self) -> int:
        ...
        """
        return len(self._data[0].features[0]) if len(self._data) > 0 and self._data[0].features is not None else None  # CHANGED
    ...
    # I've turned off feature scaling, so this function is not changed.
